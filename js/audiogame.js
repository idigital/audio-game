...
audiogame.successCount = 5; // storing the success count of last 5 results
		...
		if (audiogame.isRecordMode) {
			if (e.which == 186) {
				...
				for ( var i in audiogame.musicNotes) {
					musicNotesString += audiogame.musicNotes[i].time
							+ ","
							+ audiogame.musicNotes[i].line
							+ ";";
				}
			...
			for ( var i in audiogame.dots) {
				...
				if (hitLine == audiogame.dots[i].line
					...
					audiogame.successCount = Math.min(5,
							audiogame.successCount);
...
function gameloop() {
	...
	for ( var i in audiogame.dots) {
		...
		ctx.drawImage(audiogame.dotImage, -audiogame.dotImage.width / 2,
				-audiogame.dotImage.height / 2);
		ctx.restore();
	}
